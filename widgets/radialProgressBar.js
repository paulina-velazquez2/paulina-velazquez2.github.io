    ...
    $.fn.radialPieChart = function(func, options) {
        if (func === "init") {
            ...
            for (var i = 0; i < options.data.length; i++) {
                $(this).data("__pieChartSegment" + i, new radialProgress($(this), $.extend(options, options.data[i], {
                    'lines': options.data.length,
                    'line': i
                })));
                $(this).data("__pieChartSegment" + i).toPerc({
                    'perc': sum,
    ...
    $.fn.radialMultiProgress = function(func, options) {
        if (func === "init") {
            var space = options.space || 2,
                ...
                margin = 0;
            for (var i = 0; i < options.data.length; i++) {
                $(this).data("__multiProgress" + i, new radialProgress($(this), $.extend(options, options.data[i], {
                    'fill': segmentFill,
                    'margin': margin,
                    'lines': options.data.length,
                    'line': i
